...
{
    /// <summary>
    /// Displays the latest dealt-damage event as a short-lived text indicator.<br/>
    /// Typical usage: place this on a HUD object, assign <see cref="DealtDamageModel"/>, and connect a <see cref="TextMeshProUGUI"/> plus optional <see cref="CanvasGroup"/> so the local player sees hit confirmation without coupling networking code to a specific window.<br/>
    ...
    /// </summary>
    public sealed class DealtDamageTextView : MonoBehaviour, IObserver<DealtDamageEntry>
    {
        [Header("Model")]
        [SerializeField]
        [Tooltip("Owner-local dealt-damage model published by the networking bridge.")]
        private DealtDamageModel dealtDamageModel;

        [Header("UI")]
        [SerializeField]
        [Tooltip("Text element used to render the latest dealt-damage amount.")]
        ...
        [Tooltip("Total time in seconds that the dealt-damage text remains visible after a hit.")]
        private float displayDurationSeconds = 0.75f;

        [SerializeField, Min(0f)]
        [Tooltip("How many seconds at the end of the display window are used to fade the text out.")]
        private float fadeOutDurationSeconds = 0.25f;
        ...
        [SerializeField]
        [Tooltip("If true, append a kill marker suffix when the hit is fatal.")]
        private bool showFatalSuffix = true;
        ...
        /// <summary>
        /// Subscribes to dealt-damage events when the component becomes active.<br/>
        ...
        private void OnEnable()
        {
            ...
            {
                Debug.LogError($"[{nameof(DealtDamageTextView)}] Missing required reference on '{gameObject.name}': {nameof(dealtDamageModel)}.", gameObject);
                return;
            }

            if (damageText == null)
            {
                Debug.LogError($"[{nameof(DealtDamageTextView)}] Missing required reference on '{gameObject.name}': {nameof(damageText)}.", gameObject);
                return;
            }
        ...
        /// <summary>
        /// Unsubscribes from dealt-damage events when the component is disabled.<br/>
        ...
        /// <summary>
        /// Updates the dealt-damage indicator fade over time after a hit has been shown.<br/>
        ...
        /// <summary>
        /// Receives a newly published dealt-damage event and updates the text indicator.<br/>
        ...
        /// </summary>
        /// <param name="entry">The dealt-damage event to display.</param>
        public void OnNotify(DealtDamageEntry entry)
        {
            if (entry.DamageAmount <= 0 && entry.TargetConnectionId == 0 && entry.TargetObjectId == 0 && string.IsNullOrEmpty(entry.WeaponIconKey) && !entry.IsFatal)
                return;
        ...
        /// <summary>
        /// Builds the on-screen text for a dealt-damage event.<br/>
        /// Typical usage: called when a new successful hit arrives so the latest dealt damage can be rendered consistently.<br/>
        /// Configuration/context: optional weapon/effect attribution and fatal suffix are included only when enabled and applicable.
        /// </summary>
        /// <param name="entry">The dealt-damage event to format.</param>
        /// <returns>The formatted dealt-damage text for display.</returns>
        private string BuildDisplayText(DealtDamageEntry entry)
        {
            string amountText = $"+{entry.DamageAmount}";
            if (showWeaponIconKey && !string.IsNullOrWhiteSpace(entry.WeaponIconKey))
                amountText = $"{entry.WeaponIconKey} {amountText}";

            if (showFatalSuffix && entry.IsFatal)
